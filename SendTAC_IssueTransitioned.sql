...
--! SQL для Issue without Epic

WITH
-- Определяем alias для ключевого события
  '4f68c704-d400-48f0-afbf-e6ee0a46372a'::UUID as issue_transitioned,
  
-- Создаем флаг для определение инициатора из списка ботов,роботов
  lower(jc.created_by) IN ('robotjira@infomaximum.com','automation_bot@infomaximum.com') AS created_by_bot,
  ...
  -- Если был совершен переход в статус "Закрыто", вытягиваем из JSON значение для поле "Решение", в противном случае null       
    nullIf(
      if(JSON_VALUE(status_change_json, '$.toString') = 'Закрыто',
         JSON_VALUE(resolution_json, '$.toString'),
         null),
      ''
    ) AS resolution,
  ...
  -- Собираем массив с наблюдателями для эпика (через ответ REST API)
  --   Нужно для увеличение глубины анализа с целью сбора получателей для уведомления. На первом этапе отказались от этого, в будущем нужно реализовать через доп. функцию, чтобы пользователь мог самостоятельно выбирать
--   arrayMap(
--     watcher -> JSONExtractString(watcher, 'emailAddress'),
--     JSONExtractArrayRaw(JSONExtractRaw(reponse-from-deleted-block, 'watchers'))
--   ) AS epic_watchers,
  ...
  (
    ...
    (
      ...
      WHERE is_active = 1
            AND event_type_id = issue_transitioned
            AND lower(email) IN (
          SELECT 
            lower(email_address)
          FROM 
  ...
  -- Разрешённые email-адреса:
  --  Желание получать уведомления от ботов,роботов
  (
  SELECT
   arrayDistinct(groupArray(lower(email)))
   FROM teams_notification_subscriptions 
...
--     if(isNotNull(epic_watchers), arrayMap(watcher -> lower(watcher), epic_watchers), []),
    ...
    [lower(jc.issue_assignee)],
    [lower(jc.issue_reporter)]
--     [lower(fields.Менеджер Продаж.emailAddress)],
--     [lower(fields.Аналитик.emailAddress)],
--     [lower(fields.Специалист ТП.emailAddress)],
--     [lower(fields.Менеджер внедрений.emailAddress)],
--     [lower(fields.reporter.emailAddress)],
--     [lower(fields.assignee.emailAddress)]
  ]) AS combined_watchers_list,
  ...
  arrayDistinct(
    arrayFilter(
      email -> (
        ...
        AND email != ''
        AND email IS NOT NULL
        AND arrayExists(v -> v = email, valid_recipients)
        AND (created_by_bot ? arrayExists(v -> v = email, bot_recipients) : 1)
      ),
      combined_watchers_list
    )
  ) AS targetEmailsArray
